...
export const RadialFlow = ({ isPreview = false }: RadialFlowProps) => {
  ...
  const initialNodes: Node[] = useMemo(() => {
    ...
    const obsidianY = isPreview ? (isMobileCollapsed ? 50 : 120) : 200;
    nodes.push({
      id: 'obsidian-vault',
      type: 'obsidian',
    ...
    const claudeY = isPreview ? (isMobileCollapsed ? 50 : 120) : 200;
    nodes.push({
      id: 'claude-center',
      type: 'claude',
    ...
    return (
      <div className="w-full h-[180px] flex items-center justify-center bg-gradient-surface overflow-hidden">
        <ReactFlow
          ...
          nodeTypes={nodeTypes}
          fitView={!isMobile}
          defaultViewport={isMobile ? { x: 0, y: -50, zoom: 0.8 } : undefined}
